...
Splattie's bar for a **drop-in** asset generator is narrow: **one image in → feed-forward (no per-subject training) → a 3D Gaussian Splatting asset rigged to a model the widget animates client-side (FLAME / SMPL-X / SMAL / arbitrary-skeleton LBS) → under a commercially-usable license.** Most published avatar/object work misses at least one axis — multi-view or video capture, per-subject optimization, a mesh / NeRF / tri-plane instead of splats, or a research-only license.

**Splattie's baselines:**

| Asset type | Pipeline | License |
|------------|----------|---------|
| **Heads** | [LAM](https://github.com/aigc3d/LAM) — single image → feed-forward → FLAME-rigged 3DGS | Apache 2.0 |
| **Bodies** | [LHM](https://github.com/aigc3d/LHM) — single image → feed-forward → SMPL-X-rigged 3DGS | Apache 2.0 (weights CC BY-NC) |
| **Objects** | [TRELLIS](https://github.com/microsoft/TRELLIS) → [Puppeteer](https://github.com/Seed3D/Puppeteer) — image → native 3DGS, then auto skeleton + skinning | MIT / Apache 2.0 |
| **Quadruped mammals** | [TripoSplat](https://github.com/VAST-AI-Research/TripoSplat)/TRELLIS → [SuperAnimal](https://github.com/DeepLabCut/DeepLabCut) → [SMAL](https://smal.is.tue.mpg.de/) — reconstruct splat → keypoints → SMAL fit | MIT + research-only models |

**→ [Full method landscape — ~115 head / body / object / animal approaches compared](docs/METHODS.md)**, each scored against the drop-in bar with the reason it does or doesn't fit. The takeaway: LAM / LHM are the only commercially-clean single-image feed-forward splat generators today — the recent 2025–26 wave (FlexAvatar, Avat3r, UIKA, IDOL, …) is uniformly either no-code-yet or non-commercial.
